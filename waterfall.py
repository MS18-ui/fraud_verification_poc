...
# Explicit life-cycle-stage values a caller can set on the request, per the
# 2026-08-08 working call: the API payload should carry which stage of the
# relationship this request is for (onboarding vs. an ongoing transaction),
# rather than us purely inferring it from which fields happen to be present.
# This is also the natural place a future per-stage vendor-trust config
# would key off of ("we might want different sources for different
# life cycle stages") -- not built yet, this is the first step: an
# explicit, honest signal to build that on top of.
LIFE_CYCLE_STAGE_TO_TRANSFER_TYPE = {
    "onboarding": ONBOARDING,
    "domestic_transaction": DOMESTIC,
    "international_transaction": INTERNATIONAL,
}


def classify_transfer_type(request: dict) -> str:
    """Determines which route a request uses.

    Prefers the explicit `life_cycle_stage` field when the caller supplies
    one (see LIFE_CYCLE_STAGE_TO_TRANSFER_TYPE above) -- this is the
    "explicit indicator" the orchestration layer should be driven by,
    rather than us purely inspecting which fields are present. Falls back
    to the original field-presence inference when life_cycle_stage is
    absent, so every existing caller/fixture is unaffected:

      - swift_code present, routing_number absent -> international
      - routing_number AND account_number present  -> domestic
      - neither                                     -> onboarding
    """
    life_cycle_stage = (request.get("life_cycle_stage") or "").strip().lower()
    if life_cycle_stage in LIFE_CYCLE_STAGE_TO_TRANSFER_TYPE:
        return LIFE_CYCLE_STAGE_TO_TRANSFER_TYPE[life_cycle_stage]
