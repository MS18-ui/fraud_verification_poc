...
<style>
    ...
    .step-trail-card {{
        background: {CARD_BG}; border: 1px solid {CARD_BORDER}; border-radius: 10px;
        padding: 18px 22px 14px; margin: 18px 0 20px;
    }}
    .step-trail-title {{
    ...
    .step-trail-label {{
        ...
        margin-top: 8px; padding: 0 4px; max-width: 100px;
    }}
    .step-trail-connector {{
        flex: 1 1 24px; height: 2px; margin-top: 15px; min-width: 20px;
        border-radius: 2px;
    }}
</style>
...
if result:
    ...
    steps = result["steps"]
    trail_parts = ['<div class="step-trail-card">', '<div class="step-trail-title">Pipeline Steps</div>', '<div class="step-trail">']
    for i, step in enumerate(steps):
        label = LAYER_LABELS_SHORT.get(step["step"], step["step"])
        color = STEP_STATUS_COLOR.get(step["status"], "#9aa4b1")
        glyph = STEP_STATUS_ICON.get(step["status"], str(i + 1))
        trail_parts.append(f'''
        <div class="step-trail-item">
            <div class="step-trail-circle" style="background:{color};">{glyph}</div>
            <div class="step-trail-label" style="color:{color};">{label}</div>
        </div>''')
        if i < len(steps) - 1:
            trail_parts.append(f'<div class="step-trail-connector" style="background:{color}55;"></div>')
    trail_parts.append("</div></div>")
