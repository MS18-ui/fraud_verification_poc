...
has produced a canonical dict. Wires together:
  identity.yaml + owner.yaml + account.yaml + reasonability.yaml -> per-domain outcome
  supplementary.yaml                          -> flags + outcome caps/overrides
  decision_table.yaml                         -> overall decision (identity/owner/account
                                                  only -- reasonability deliberately never
                                                  gates the overall decision, see
                                                  rules/reasonability.yaml)
...
def run_decision(canonical: dict) -> dict[str, Any]:
    ...
    reasonability_doc = _load("reasonability.yaml")
    ...
    reasonability_outcome, reasonability_rule = evaluate_domain(reasonability_doc, canonical, COMPUTED_REGISTRY)

    primary_outcomes = {
        "identity": identity_outcome,
        "owner": owner_outcome,
        "account": account_outcome,
        "reasonability": reasonability_outcome,
    }
    primary_rule_ids = {
        "identity": identity_rule,
        "owner": owner_rule,
        "account": account_rule,
        "reasonability": reasonability_rule,
    }
