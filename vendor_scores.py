...
def ekata_score(raw: Optional[dict]) -> dict[str, Any]:
    ...
    phone = raw.get("primary_phone_checks") or {}
    address = raw.get("primary_address_checks") or {}
    email = raw.get("primary_email_address_checks") or {}
    ip = raw.get("ip_address_checks") or {}
    if phone.get("match_to_name") is not None:
        checks.append({"label": "Phone match to name", "value": phone["match_to_name"]})
    if phone.get("line_type") is not None:
        checks.append({"label": "Phone line type", "value": phone["line_type"]})
    if address.get("match_to_name") is not None:
        checks.append({"label": "Address match to name", "value": address["match_to_name"]})
    if address.get("type") is not None:
        checks.append({"label": "Address type", "value": address["type"]})
    if email.get("match_to_name") is not None:
        checks.append({"label": "Email match to name", "value": email["match_to_name"]})
    if email.get("is_disposable") is not None:
        checks.append({"label": "Email disposable", "value": "Yes" if email["is_disposable"] else "No"})
    if ip.get("proxy_risk") is not None:
        checks.append({"label": "IP proxy risk", "value": "Yes" if ip["proxy_risk"] else "No"})
